...
def get_operator(h, opname, projector=False):
    """Return an operator"""
    if opname == "None":
        op = None  # no operators
    elif opname == "Sx":
        op = h.get_operator("sx")  # off plane case
    elif opname == "Berry":
        op = h.get_operator("berry")  # off plane case
    elif opname == "Sy":
        op = h.get_operator("sy")  # off plane case
    elif opname == "Sz":
        op = h.get_operator("sz")  # off plane case
    elif opname == "Valley":
        op = h.get_operator("valley", projector=projector)
    elif opname == "IPR":
        op = h.get_operator("ipr")
    elif opname == "y-position":
        op = h.get_operator("yposition")
    elif opname == "x-position":
        op = h.get_operator("xposition")
    elif opname == "z-position":
        op = h.get_operator("zposition")
    elif opname == "Interface":
        op = h.get_operator("interface")
    elif opname == "Surface":
        op = h.get_operator("surface")
    elif opname == "Layer":
        op = h.get_operator("zposition")
    else:
        op = h.get_operator(opname)
    return op
...
def get_bands(h, window):
    ...
    if num_bands < 1:
        num_bands = None  # all the eigenvalues
...
def get_dos(h, window, silent=False):
    ...
    energies = np.linspace(-ewindow, ewindow,
                           int(ewindow/delta*5))  # get the energies
    if window.getbox("dos_mode") == "Green":
        dos.dos(h, delta=delta, nk=nk, energies=energies,
                mode="Green")  # compute DOS
    else:
...
def get_multildos(h, window):
    ...
    if proj == "Real space atomic orbitals":
        projection = "atomic"
    else:
        projection = "TB"  # default one
    ...
    if projection == "TB":
        ...
        execute_script("qh-multildos --grid True")
